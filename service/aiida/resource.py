...
class BaseResource(Resource):
    ...
    ]  # Close the thread's storage connection after any method call
    ...
    def __init__(self, profile, **kwargs):
        ...
        if "get_decorators" in kwargs and isinstance(
            kwargs["get_decorators"], (tuple, list, set)
        ):
    ...
    def _load_and_verify(self, node_id=None):
        ...
        if not isinstance(
            node, self.trans._aiida_class
        ):  # pylint: disable=protected-access,isinstance-second-argument-not-valid-type
    ...
    def get(
        ...
        if query_type == "projectable_properties":
            ...
            (
                ...
                ordering,
            ) = self.trans.get_projectable_properties()
            ...
            self.trans.set_query(
                filters=filters, orders=orderby, node_id=node_id
            )
...
class QueryBuilder(BaseResource):
    ...
    def __init__(self, **kwargs):
        ...
        if "get_decorators" in kwargs and isinstance(
            ...
            self.method_decorators.update(
                {"post": list(kwargs["get_decorators"])}
            )
    ...
    def post(self):  # pylint: disable=too-many-branches
        ...
        self.trans.__label__ = self.trans._result_type = self.trans._query_help[
            "path"
        ][-1]["tag"]
        ...
        skip_tags = []
        for tag, projections in tuple(
            self.trans._query_help["project"].items()
        ):
            if projections == [{"*": {}}]:
        ...
        if empty_projections_counter == number_projections:
            ...
            self.trans._query_help["project"][
                self.trans.__label__
            ] = self.trans._default
        ...
        headers = self.utils.build_headers(
            url=request.url, total_count=self.trans.get_total_count()
        )
        return self.utils.build_response(
            status=200,
            headers=headers,
    ...
    def get(
        ...
        ## Treat the statistics
            ...
            if filters:
                ...
                user_pk = None
            results = self.trans.get_namespace(
                user_pk=user_pk, count_nodes=True
            )
        ...
        # TODO add pagination for this endpoint (add default max limit)
            ...
            headers = self.utils.build_headers(url=request.url, total_count=0)
            results = self.trans.get_io_tree(
                node_id, tree_in_limit, tree_out_limit
            )
            ...
            if page is not None:
                ...
                if (
                    query_type == "download"
                    and download not in ["false", "False", False]
                    and results
                ):
            ...
            if attributes_filter is not None and attributes:
                for node in results["nodes"]:
                    ...
                    for attr in attributes_filter:
                        node["attributes"][str(attr)] = node[
                            f"attributes.{str(attr)}"
                        ]
...
class CalcJobNode(ProcessNode):
    ...
    from aiida.restapi.translator.nodes.process.calculation.calcjob import (
        CalcJobTranslator,
    )
