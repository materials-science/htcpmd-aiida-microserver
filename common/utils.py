...
class CustomJSONEncoder(JSONEncoder):
    ...
    This has to be provided to the Flask service in order to replace the default
    encoder.
    """
    ...
    def default(self, o):
        ...
        if isinstance(o, datetime):
            if (
                    "datetime_format" in SERIALIZER_CONFIG
                    and SERIALIZER_CONFIG["datetime_format"] != "default"
            ):
...
class Utils:
    ...
    def validate_request(
        ...
        if perpage is not None and (limit is not None or offset is not None):
            raise RestValidationError(
                "perpage key is incompatible with limit and offset"
            )
        ...
        if page is not None and (limit is not None or offset is not None):
            raise RestValidationError(
                "requesting a specific page is incompatible with limit and "
                "offset"
            )
        ...
        if query_type in ("projectable_properties",) and is_querystring_defined:
            raise RestInputValidationError(
                "projectable_properties requests do not allow specifying a "
                "query string"
            )
    ...
    def paginate(self, page, perpage, total_count):
        ...
        if page > last_page or page < 1:
            raise RestInputValidationError(
                f"Non existent page requested. The page range is ["
        ...
        rel_pages = dict(
            prev=prev_page, next=next_page, first=first_page, last=last_page
        )
    ...
    def build_headers(self, rel_pages=None, url=None, total_count=None):
        ...
        next, last)
        :param url: (string) the full url, i.e. the url that the client uses
        to get Rest resources
        """
        ...
        if rel_pages is not None and url is None:
            raise InputValidationError(
                "'rel_pages' parameter requires 'url' parameter to be defined"
            )
        ...
        def make_rel_url(rel, page):
            ...
            return f"<{'/'.join(new_path_elems)}{question_mark}" \
                   f"{query_string}>; rel={rel}, "
    ...
    @staticmethod
    def build_response(status=200, headers=None, data=None, code=0, msg=""):
        """
        Build the response
    ...
    def build_translator_parameters(self, field_list):
        ...
        if (
                "download_format" in field_counts
        ...
        if (
                "attributes_filter" in field_counts
        ...
        if "profile" in field_counts and field_counts["profile"] > 1:
            raise RestInputValidationError(
                "You cannot specify profile more than once"
            )
        ...
        for field in field_list:
            if field[0] == "profile":
                if field[1] == "=":
                    ...
                    raise RestInputValidationError(
                        "only assignment operator '=' is permitted after "
                        "'profile'"
                    )
            elif field[0] == "limit":
                if field[1] == "=":
                ...
                if field[1] == "=":
                    # Consider value (gives string) and value_list (gives
                    # list of
                    # strings) cases
                ...
                if (
                        isinstance(field_value, DatetimePrecision)
                        and operator == "="
                ):
    ...
    def parse_query_string(self, query_string):
        ...
        value_bool = (Literal("true") | Literal("false")).addParseAction(
        ...
        value_list = Group(
            value + OneOrMore(Suppress(",") + value) + Optional(Suppress(","))
        )

        # Fields
        single_field = Group(key + operator + value)
        list_field = Group(
            key + (Literal("=in=") | Literal("=notin=")) + value_list
        )
        ...
        try:
            ...
            # JQuery adds _=timestamp a parameter to not use cached
            # data/response.
    ...
    @staticmethod
    def get_header(req: request, key: str):
        value = req.headers.get(key, None)

        return urllib.parse.unquote(value) if value is not None else value

    @staticmethod
    def get_token(req: request):
        token = req.headers.get(TokenConstant.AUTHENTICATION)
    ...
    def parse_token(token: str) -> Dict[str, Any]:
        return jwt.decode(token,
                          key=TokenConstant.TOKEN_SECRET,
                          algorithms=TokenConstant.ALGORITHMS,
                          options={
...
def list_routes():
    ...
    for rule in current_app.url_map.iter_rules():
        ...
        line = urllib.parse.unquote(f"{methods:20s}: {rule}")

        if rule.endpoint in output:
            output.get(rule.endpoint).append(line)
        else:
            output.setdefault(rule.endpoint, [])

    sorted_keys = sorted(output.keys())
    for key in sorted_keys:
        output.setdefault(key, sorted(set(output.get(key))))

    return output


def add_endpoint_prefix(prefix: str, endpoint: str) -> str:
...
def close_thread_connection(wrapped, _, args, kwargs):
    ...
    It is needed due to the server running in threaded mode, i.e.,
    creating a
    new thread for each incoming request,
    and leaving connections unreleased.

    Note, this is currently hard-coded to the `PsqlDosBackend` storage
    backend.
    """
    try:
