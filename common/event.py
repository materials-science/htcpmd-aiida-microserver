...
class EventUtil:
    ...
    def _get_event_user_info(headers: dict):
        ...
        if user_id:
            UserContextHolder.set_user_id(user_id)
        user_key = headers.get(UserConstant.USER_KEY_FIELD_NAME)
        if user_key:
            UserContextHolder.set_user_key(user_key)
        user_name = headers.get(UserConstant.USERNAME_FIELD_NAME)
        if user_name:
            UserContextHolder.set_username(user_name)
        user_ip = headers.get(UserConstant.REQUEST_ADDR_FIELD_NAME)
        if user_name:
            UserContextHolder.set(UserConstant.REQUEST_ADDR_FIELD_NAME, user_ip)
        ...
        UserContextHolder.set("headers", headers)
    ...
    def pre_event_handler(*args):
        ...
        def decorator(func):
            def _handler(body):
                ...
                return func(data=data, headers=headers, message_id=message_id)
